...
```{r}
census_api_key("5af8c3de403f054574b75e677bd81ea869925945", overwrite = TRUE)
```
...
acs_variable_list <- load_variables(2020, #year
                                    "acs5", #five year American Community Survey estimates
                                    cache = TRUE)
```
...
They are hosted at this URL - council_dist <- st_read("https://raw.githubusercontent.com/mafichman/cpln_0795/main/data/council_dist.geojson")

```{r}
# Chunk 2
# Get hydrology data
philly_water <- area_water(state = "PA", county = "Philadelphia", class = "sf", year = 2020)
...
```{r}
council_dist <- st_read("https://raw.githubusercontent.com/mafichman/cpln_0795/main/data/council_dist.geojson")
...
ggplot() +
  geom_sf(data = acs2020_incidents,
          aes(fill = n)) +
  geom_sf(data = philly_water, fill = "skyblue") +
  geom_sf(data = council_dist, fill = "transparent", color = "grey") +
  labs(
    fill = "Incidents",        # Legend title for pct_wht
    title = "Incidents by tract",
    subtitle = "with water features",
